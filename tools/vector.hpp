...
namespace machinelearning { namespace tools {
    ...
    class vector {
        ...
        public :
            ...
            template<typename T> static ublas::vector<std::size_t> rankIndex( ublas::vector<T>& );
    ...
    **/
     ...
     * @return index vector
     **/
    template<typename T> inline ublas::vector<std::size_t> vector::rankIndex( ublas::vector<T>& p_vec )
    {
        std::vector<std::size_t> temp(boost::counting_iterator<std::size_t>(0), boost::counting_iterator<std::size_t>(p_vec.size()));
        std::sort(temp.begin(), temp.end(), lam::var(p_vec)[lam::_1] < lam::var(p_vec)[lam::_2]);
