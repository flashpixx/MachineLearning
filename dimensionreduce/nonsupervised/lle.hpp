...
#ifndef MACHINELEARNING_DIMENSIONREDUCE_NONSUPERVISED_LLE_HPP
#define MACHINELEARNING_DIMENSIONREDUCE_NONSUPERVISED_LLE_HPP
...
namespace machinelearning { namespace dimensionreduce { namespace nonsupervised {
    ...
    /** create the Local Linear Embedding (LLE) from data **/
    ...
    template<typename T> inline ublas::matrix<T> lle<T>::map( const ublas::matrix<T>& p_data )
    {
        if (p_data.size2() <= m_dim)
            throw exception::runtime(_("data points are less than target dimension"));
